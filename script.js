...
function handleSearch() {
    ...
    let results = searchJobs(title, location);

    let jobResults = document.getElementById("jobResults");
    jobResults.innerHTML = "";
    
    if (results.length === 0) {
        jobResults.innerHTML = "<li>Nessun lavoro trovato</li>";
    } else {
        for (let i = 0; i < results.length; i++) {
            let li = document.createElement("li");
            li.textContent = results[i].title + " - " + results[i].location;
...
function searchJobs(title, location) {
    ...
    while (i < jobs.length) {
        ...
        if (jobTitle.includes(lowerTitle) && jobLocation.includes(lowerLocation)) {
            result.push(jobs[i]);
        }
